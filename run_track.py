...
def change_track_index(x):
    global track_index
    track_index = x
    if WITH_QT:
        cv2.displayOverlay(WINDOW_NAME, f"Selected track id {track_index}/{last_track_index}\n {track_ids[track_index]}", 3000)
    else:
        print(f"Selected track: {track_ids[track_index]}")
...
def yolo_format(class_index, track_index, point_1, point_2, width, height):
    ...
    return str(class_index) + " " + str(track_index) + " " + str(x_center) \
...
def draw_bboxes_from_file(tmp_img, txt_path, width, height):
    ...
    if os.path.isfile(txt_path):
        ...
        for line in content:
            ...
            if args.format == 'yolo':
                class_index, track_index, x_center, y_center, x_width, y_height = map(float, values_str)
            ...
            tmp_img = draw_text(tmp_img, f"{track_ids[track_index]}: {class_list[class_index]}", (x1, y1 - 5), color, args.bbox_thickness)
    return tmp_img
...
last_track_index = len(track_ids) - 1
...
while True:
    ...
    if point_1[0] != -1:
        ...
        cv2.line(tmp_img, (0, mouse_y), (width, mouse_y), color, 1)
        cv2.line(tmp_img, (mouse_x, 0), (mouse_x, height), color, 1)
        # if second click
        if point_2[0] != -1:
            # save the bounding box
            if args.format == 'yolo':
                line = yolo_format(class_index, track_index, point_1, point_2, width, height)
            elif args.format == 'voc':
            ...
            if WITH_QT:
                cv2.displayOverlay(WINDOW_NAME, "Selected label: " + class_list[class_index] + ""
                                   ...
                                   120)
                cv2.displayOverlay(WINDOW_NAME, "Selected track id: " + track_ids[track_index] + ""
                                                                                               "\nPress [W] or [S] to change.",
                                   120)
    ...
    if pressed_key == ord('a') or pressed_key == ord('d'):
        ...
        # change down current track id key listener
        if pressed_key == ord('S'):
            track_index = decrease_index(track_index, last_track_index)
        # change up current track id key listener
        elif pressed_key == ord('W'):
